...
// 1️⃣ DECLARAR PRIMERO las variables globales que se usan en todo el archivo
let datosFormularios = {};  // ← USA let, NO const, y declárala ANTES
...
//   MAPEOS PARA LA FUNCION
    ...
    function setSafeDisplay(id, mostrar) {
        ...
        if (elemento) {
            ...
            console.warn(`⚠️ Elemento con id="${id}" no encontrado para ocultar/mostrar`);
        }
    }

    function guardarDatosFormulario(numForm) {

    // ==================== EXCEPCIÓN FORM4 (texto del label) ====================
    if (numForm === 4) {
        ...
        datosFormularios.form4 = {
            ...
            codigoBastidor: codigoBastidor  // '0301', '0302' o '0401' ← CÓDIGO LISTO
        };
        
        console.log(`✅ Form${numForm} guardado:`, JSON.stringify(datosFormularios.form4, null, 2));
        return; // Salir para no ejecutar código general
    }


    // ==================== EXCEPCIÓN FORM5 (radio buttons con estructura compleja) ====================
    if (numForm === 5) {
        ...
        const labelText = selected.parentElement.querySelector('p').textContent.trim();
        const codigoEmb = CODIGOS_EMBELLECEDOR[value];    // Transformar a código de embellecedor
  
        datosFormularios.form5 = {
            division: {
                tipo: value,
                texto: labelText,
                codigoEmb: codigoEmb,  // '', 'MA', 'MB', 'MC' o 'MD' ← CÓDIGO LISTO
                lineas: value === 'D' 
                    ? [
        ...
        console.log(`✅ Form${numForm} guardado:`, JSON.stringify(datosFormularios.form5, null, 2));
        return; // Salir para no ejecutar código general
    }

        // ==================== EXCEPCIÓN FORM6 (mapeo de acabado y vidrio) ====================
    if (numForm === 6) {
        ...
        datosFormularios.form6 = {
            ...
            // NUEVOS: Códigos para la función de precios
            codigoColor: codigoColor,        // 'L.BM03'
            codigoVidrio: codigoVidrio       // '01'
        };
        
        console.log(`✅ Form${numForm} guardado con códigos:`, datosFormularios.form6);
        return; // Salir para no ejecutar código general
    }

    // ==================== CÓDIGO GENERAL (Form1, Form2, Form3, Form6) ====================
...
function anteriorFormulario(actual) {
  ...
  if (actual === 5) {
   ...
   // Limpiar datos del formulario actual -  añadido posterior para poder navergar y reflejar cambios
...
function AnchoEleCorredera(opcion, ancho) {
 ...
 let NumPuertas = 0
 let NumFijos = 0
    // Calcular X según la opción seleccionada
  switch(opcion) {
    case '1puerta':
      X = ancho + 40;
      NumPuertas = 1
  ...
  return { X, retracInf, NumPuertas, NumFijos };    //  es el valor devuelto de la función, para poder seguir usandola.
...
let Y = 0; // alto del elemento
  ...
  return { Y };    //  es el valor devuelto de la función, para poder seguir usandola.
}

//  8888888888888888888888888888888888888888888888888888888888888

function realizarCalculos() {
  try { // ← AÑADE ESTO AL INICIO
    console.log('🚀 INICIANDO realizarCalculos()');
  ...
  console.log('Valores extraídos:', { opcion, tipoFijo, guia, alto, ancho, acabado, textoDivision, colorVidrio });

// PASO 1: Calcular valores básicos
  ...
  // Determinar qué bloques mostrar según la opción
  ...
  // ⬇️⬇️⬇️ acciones para preparar calculos de precios  ⬇️⬇️⬇️
  ...
  const datosPuerta = {
    ...
    altura: Math.round(Y),      // Asegurar número entero
    ancho: Math.round(X),       // Asegurar número entero
  ...
  if (tieneFijo && NumFijos > 0) {
    datosFijo = {
      ...
      color: datosFormularios.form6.codigoColor,
      vidrio: datosFormularios.form6.codigoVidrio,
      altura: Math.round(resultadoFijo.altoFijo),   // Número entero
      ancho: Math.round(resultadoFijo.anchoFijo),   // Número entero
  ...
  console.log('📏 Valores calculados:', { X, Y, altoFijo: resultadoFijo?.altoFijo, anchoFijo: resultadoFijo?.anchoFijo }); // ← AÑADE ESTO
  
  
  
  // 3. Llamar a la función para PUERTA


  const resultadoPrecioPuerta = window.calcularPresupuesto(datosPuerta);

  // 4. Llamar a la función para FIJO (si existe)
  const resultadoPrecioFijo = null;
  if (datosFijo) {
    console.log('🔍 Llamando a calcularPresupuesto (FIJO):', datosFijo);
    resultadoPrecioFijo = windows.calcularPresupuesto(datosFijo);
    
    console.log('💰 Resultado precio fijo:', resultadoPrecioFijo);
  }

  // ⬆️⬆️⬆️ **FIN DE LAS LLAMADAS** ⬆️⬆️⬆️
  ...
  const datosInforme = {
    ...
    altoPuerta: Y,  // ← Guardados par el segundo informe
    anchoPuerta: X,
    ...
    window.ultimosDatosInforme = datosInforme; // ← GUARDAR AQUÍ
    return datosInforme; // DEVUELVE los datos

      } catch (error) { // ← AÑADE ESTO AL FINAL
    console.error('💥 ERROR CRÍTICO en realizarCalculos():', error);
    alert('⚠️ Error al calcular: ' + error.message);
    return null;
  }
...
// Funciones para iconos de perfiles (imágenes jpg desde GitHub)
...
function generarMensajeOpcion(opcion, tipoFijo, textoDivision, conjuntos) {
    ...
    let mensaje = comentarios[opcion] || 'CONFIGURACIÓN PERSONALIZADA';
    
    // Si tiene fijo, añadir el tipo
    if (tipoFijo && tiposFijo[tipoFijo]) {
        mensaje += tiposFijo[tipoFijo];
    }
    
    // ➜ NUEVO: Añadir texto de divisiones si existe, aunque siempre existe.
    mensaje += ` - ${textoDivision || 'Sin embellecedor PEPE'}`;

    // ➜ NUEVO: Añadir el número de conjuntos al final
...
function calcularPerfilesFijo(alto, X, NumFijos, tipoFijo) {
  ...
  // Calcular según el tipo de fijo seleccionado
  switch(tipoFijo) {
    case '0': // 3 paredes
...
function calcularEmbellecedorFijo(alto, X, acabado, tipoFijo) {
  ...
  /* tabla de fórmulas con alto bruto */
  const f = {
...
function mostrarInformesCompletos() {
  try {
  console.log('🔵 Iniciando mostrarInformesCompletos()');
  ...
  if (d.mostrarFijo) {
    ...
    // Añade esto justo antes de ocultar/mostrar
    //console.log('Modelo embellecedor:', modeloEmbellecedor);
   // console.log('¿Es sin embellecedor?:', esSinEmbellecedor);
  ...
  if (informeContainer) {
    informeContainer.scrollIntoView({ behavior: 'smooth' });
        ...
        console.error('💥 ERROR en mostrarInformesCompletos():', error);
...
function volverAlInicio() {
  ...
  // Limpiar últimos datos de informe
...
function llenarBloquePuerta(datos) {
  ...
  // ===== SECCIÓN EMBELLECEDOR (TRABAJA DIRECTO CON LA TABLA) =====
  const tablaEmbellecedor = document.getElementById('tabla-embellecedor-puerta');
  
  if (!tablaEmbellecedor) {
    console.warn('⚠️ tabla-embellecedor-puerta no encontrada');
    return;
  }
...
function llenarBloqueFijo(datos) {
  // Perfiles y cristales (código existente)
  ...
  // ===== SECCIÓN EMBELLECEDOR FIJO =====
  const tablaEmbFijo = document.getElementById('tabla-embellecedor-fijo');
  
  if (!tablaEmbFijo) {
    console.warn('⚠️ tabla-embellecedor-fijo no encontrada');
    return;
  }
...
// 🚨 DIAGNÓSTICO: Verificar que app.js se ejecutó completamente
console.log('✅ DIAGNÓSTICO APP.JS');
console.log('✅ datosFormularios:', typeof datosFormularios);
console.log('✅ siguienteFormulario:', typeof window.siguienteFormulario);
console.log('✅ calcularPresupuesto:', typeof window.calcularPresupuesto);
